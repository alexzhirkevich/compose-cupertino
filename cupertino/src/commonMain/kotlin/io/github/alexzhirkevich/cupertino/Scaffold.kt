...
fun CupertinoScaffold(
    ...
    Surface(
        ...
        CompositionLocalProvider(
            LocalAppBarOpacityState provides remember { AppBarsOpacityState() },
        ) {
            ScaffoldLayout(
                fabPosition = floatingActionButtonPosition,
                topBar = {
                    CompositionLocalProvider(
                        LocalAppBarsColor provides Color.Transparent,
                        content = topBar
                    )
                },
                bottomBar = {
    ...
    SubcomposeLayout { constraints ->
        ...
        layout(layoutWidth, layoutHeight) {
            ...
            val bodyContentPlaceables = subcompose(ScaffoldLayoutContent.MainContent) {
                ...
                val isTopBarTransparent = LocalAppBarOpacityState.current?.isTopBarTransparent?.value ?: true
                val isBottomBarTransparent = LocalAppBarOpacityState.current?.isBottomBarTransparent?.value ?: true


                val ssgbColor = CupertinoTheme.colorScheme.secondarySystemGroupedBackground
                val topColor by animateColorAsState(
                    targetValue = if (isTopBarTransparent) ssgbColor.copy(alpha =  0f)
                    else ssgbColor.copy(alpha = CupertinoMaterialsAlphaThin),
                    animationSpec = spring(stiffness = Spring.StiffnessMedium)
                )
                val bottomColor by animateColorAsState(
                    targetValue = if (isBottomBarTransparent) ssgbColor.copy(alpha =  0f)
                    else ssgbColor.copy(alpha = CupertinoMaterialsAlphaThin),
                    animationSpec = spring(stiffness = Spring.StiffnessMedium)
                )

                val topModifier = if (isTopBarTransparent)
                    ...
                    backgroundColor = ssgbColor,
                    tint = topColor,
                    blurRadius = CupertinoMaterialsRadiusThin
                )

                val bottomModifier = if (isBottomBarTransparent)
                    Modifier else Modifier.haze(
                    Rect(
                        left = 0f,
...
internal class AppBarsOpacityState(
...
internal val LocalAppBarOpacityState = compositionLocalOf<AppBarsOpacityState?> {
    null
}

internal val LocalAppBarsColor = compositionLocalOf {
    Color.Unspecified
}