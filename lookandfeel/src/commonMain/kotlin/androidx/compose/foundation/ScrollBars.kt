 ...
 */
val LocalScrollbarStyle2 = staticCompositionLocalOf { defaultScrollbarStyle() }

/**
 * Defines visual style of scrollbars (thickness, shapes, colors, etc).
 * Can be passed as a parameter of scrollbar through [LocalScrollbarStyle2]
 */
//@Immutable
...
fun VerticalScrollbar(
    ...
    style: ScrollbarStyle = LocalScrollbarStyle2.current,
...
internal fun <T> OldOrNewScrollbar(
    ...
    val scrollThickness by animateIntAsState(
        if (isHighlighted)
            (style.thickness * 2.5f).roundToPx()
        else style.thickness.roundToPx()
    )

//    val scrollThickness = style.hoverThickness.roundToPx()
