...
@Composable
fun rememberSaveableStateHolder2(): SaveableStateHolder =
    rememberSaveable(
        saver = SaveableStateHolderImpl2.Saver
    ) {
        SaveableStateHolderImpl2()
    }.apply {
        parentSaveableStateRegistry = LocalSaveableStateRegistry.current
    }

private class SaveableStateHolderImpl2(
    ...
    override fun SaveableStateProvider(key: Any, content: @Composable () -> Unit) {
        ReusableContent(key) {
            val registryHolder = remember {
                ...
                registryHolders[key] ?: RegistryHolder(key)
            }
    ...
    companion object {
        val Saver: Saver<SaveableStateHolderImpl2, *> = Saver(
            save = { it.saveAll() },
            restore = { SaveableStateHolderImpl2(it) }
        )
    }
}
